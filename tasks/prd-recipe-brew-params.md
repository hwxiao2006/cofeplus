...
每条要求附验收动作，测试时照做即可；实现方式由开发自定，达标即可。

- **键盘可操作**：分区标题行可被 Tab 聚焦，Enter / 空格可切换展开收起。验收：全程不用鼠标，完成一次展开和收起。
- **状态可感知**：辅助技术（读屏）能识别分区当前是展开还是收起（如通过 `aria-expanded` 暴露）。验收：展开 / 收起后，状态标记随实际开合变化。
- **档位分组可识别**：压粉力度三个档位作为一组可被辅助技术识别，当前档位可感知（如 `role="group"` + `aria-label`、`aria-pressed`）。验收：切换档位后，选中态标记同步更新到新档位。
- **步进器可键盘操作**：萃取参数的数字输入框可聚焦、可直接键入数值。验收：Tab 聚焦输入框，键入数值后失焦生效。
- **移动端不溢出**：≤768px 时参数行纵向堆叠，六行参数、步进器、档位按钮完整可见且可点按，页面无横向滚动。验收：375px 宽度（iPhone SE 级别）展开分区逐行检查。
