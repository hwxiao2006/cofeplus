...
2. 系统打开复制弹窗。
3. 用户在有权管理印花素材的目标设备范围内搜索设备或点位，并勾选目标设备。
4. 系统基于已授权目标设备展示复制预检查结果。
5. 用户确认后，系统再次校验目标设备权限并开始复制。
6. 系统将素材复制到仍有权限的目标设备；对无权限或权限失效的目标设备不执行新增或覆盖，并反馈处理结果。

**验收标准：**
- [ ] 复制入口仅在当前用户存在可复制的其他授权设备时展示。
- [ ] 复制弹窗支持搜索设备编号与点位名称。
- [ ] 复制目标设备列表仅展示当前用户有权管理印花素材的设备。
- [ ] 复制支持多选目标设备。
- [ ] 复制前展示预检查结果，区分 `新增` 与 `覆盖`。
- [ ] 确认复制时，系统再次校验目标设备权限。
- [ ] 无权限设备不得新增或覆盖印花素材，并需向用户说明原因。
- [ ] 目标设备存在同名素材时，本次复制执行覆盖。
- [ ] 目标设备不存在同名素材时，本次复制执行新增。
- [ ] 复制成功且无失败项时，弹窗自动关闭。
- [ ] 当前用户没有任何可复制的其他授权设备时，不展示复制入口，也不弹出复制提示。
...
- FR-18：复制目标设备列表必须仅展示当前用户有权管理印花素材的设备。
- FR-19：确认复制时，系统必须再次校验目标设备权限；无权限或权限失效的设备不得新增或覆盖印花素材。
- FR-20：当当前用户没有任何可复制的其他授权设备时，页面不得展示复制入口。
- FR-21：上传成功后，如存在其他授权设备，系统可引导继续复制。
- FR-22：印花素材卡片必须保持轻量，只展示图片、名称、更新时间与操作按钮。
- FR-23：印花素材卡片不得展示名称键、联动状态、未引用提示等强业务耦合信息。
- FR-24：印花素材卡片中的操作按钮必须保持统一视觉样式，不得使用两套明显不同的按钮体系。
- FR-25：本期不得支持印花素材改名。
- FR-26：本期不得支持印花素材直接更换图片。
- FR-27：当需要修改现有素材时，必须通过“删除后重新上传”的方式完成。
- FR-28：当本地素材数据损坏时，系统必须安全回退为空库，不影响页面继续使用。
