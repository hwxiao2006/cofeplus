...
复制商品流程已拆分为独立文档，见同目录 `prd-copy-product-flow.html`；复制商品本期只支持复制到一台机器，不支持一次复制到多台机器。

商品运营标识本期只支持 `是否热销`，后台通过 `featured` 字段保存。业务标签功能本期不实现，已完成方案保留在 `codex/business-tags-archive` 分支供后续恢复评估。

该模块以商品管理页面为主入口，通过三个内层 Tab 组织不同工作区：

- **菜单管理**：分类导航 + 商品卡片网格 + 共享筛选，完成日常商品上下架、编辑、排序和分类维护。
- **基本设置**：设备语言配置、点单屏联系信息、售价币种。
- **批量改价**：复用共享筛选条件，对选中商品进行批量固定改价或批量上下架。

商品编辑通过商品详情页完成，支持多语言编辑、本地图片上传、配方配置和是否热销配置。
...
- 让编辑商品流程支持维护基础信息、配方配置和是否热销状态。
- 让分类管理支持新增、编辑多语言名称与图标、删除时商品迁移。
- 让基本设置将语言、联系信息和币种集中管理，并联动点单屏预览。
...
- [ ] 上架和下架按钮使用不同状态色区分。
- [ ] `featured = true` 的商品在卡片图片上方展示 `热销` 标识。
...
**描述：** 作为运营人员，我希望点击编辑后跳转到商品详情页，完成多语言商品名、价格、图片和是否热销等信息的编辑，这样我可以维护完整的商品数据。

**主流程：**
1. 用户点击商品卡片上的 `编辑` 按钮。
2. 系统进入商品详情页，并保留商品数据和当前筛选状态。
3. 用户在商品详情页编辑基本信息、配方配置和是否热销状态。
4. 用户点击 `保存` 提交修改。
...
- [ ] 基本信息包含商品图片（URL 或本地上传）、商品价格、价格对比文案、商品状态、是否热销。
- [ ] 是否热销保存到 `featured`，商品管理卡片和点单屏预览按该字段展示 `热销`。
- [ ] 商品售价统一展示基础售价与价格对比文案，不再区分税前税后。
...
> 本次状态：未实现，暂不纳入本次提测与验收范围；商品所属分类调整、分类排序调整、商品排序调整相关需求均保留为后续版本待办。
...
**描述：** 作为运营人员，我希望在基本设置 Tab 中统一管理设备语言、点单屏联系信息和币种，这样我可以集中完成设备的菜单配置。

**主流程：**
1. 用户切换到 `基本设置` Tab。
2. 用户配置设备支持语言、联系信息和售价币种。
3. 用户点击 `保存基础设置` 统一保存。
4. 用户可点击 `预览点单屏` 查看配置效果。

**验收标准：**
- [ ] 基本设置包含设备语言、点单屏联系信息和售价币种配置。
- [ ] 桌面端设置卡片改为两列布局。
...
- [ ] `featured = true` 的商品展示 `热销` 标识，非热销商品不展示。
...
- FR-11：`featured = true` 的商品必须展示 `热销` 标识。
- FR-12（后续版本）：商品所属分类调整本次不纳入验收。
- FR-13：编辑商品必须支持是否热销配置。
- FR-14：商品售价必须统一展示基础售价与价格对比文案，不再区分税前税后。
...
- FR-46：点单屏预览必须按 `featured` 展示热销标识。
...
| 热销展示 | 保存商品 | 只保存 `featured`，热销商品展示 `热销` 标识 | 商品管理和点单屏口径一致 |
...
| 价格对比文案校验 | 固定改价 | 价格对比文案可不填，按点单屏展示规则处理 | 不合法行做部分失败 |
| 联系信息未保存 | 切换设备 | 提供保存、放弃、取消三个选项 | 防止意外丢失编辑 |
| 预览语言联动 | 切换预览语言 | 刷新商品文案，不改写设备默认语言 | 预览与配置独立 |
| 业务标签范围 | 本期需求 | 业务标签管理与绑定本期不实现 | 后续从恢复分支重新评估 |
