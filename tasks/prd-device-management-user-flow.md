...
- 让设备详情首屏形成“概览、状态、运营摘要”的快速判断区，并把入场、广告屏、技术状态和记录摘要沉淀到右侧二级入口。
...
**功能说明：** 点击 `+ 设备录入` 后进入最新设备入场页，完成新设备入场配置。当前最新版不是“旧的简化录入表单”，而是按当前登录商户自动限定设备、点位和员工范围，并在入场阶段一并完成支付方式、广告屏和点位照片配置。

**当前页面表现：**
- 设备、点位、员工都按当前登录商户范围过滤，不再在页面上单独选择所属客户。
- 基本信息区包含 `选择设备`、`点位名称`，并支持 `+ 新增点位` 快速创建并自动选中。
- 页面拆成左右双栏：左侧负责设备、运维、定位和其他设置；右侧负责支付方式、广告屏设置、点位照片与提交动作。
- 提交后会保存入场信息，并同步更新设备商户、点位、入场状态、停卖状态和心跳初始状态。

![F-004 添加设备（最新分支结构预览）](../screenshots/device-prd/uf011-device-entry-latest.svg)

### F-005：设备详情摘要

**功能说明：** 设备详情弹层是设备管理的核心查看面。最新版本采用“状态首屏 + 操作优先”的信息层级，首屏只保留设备概览、设备状态和运营摘要，让用户先判断这台设备是谁、在哪里、是否正常，再通过右侧入口进入二级信息。

**当前页面表现：**
- 详情为 `左主内容 + 右侧操作区` 布局。
- 左侧首屏只显示 `设备概览`、`设备状态`、`运营摘要` 三张卡片，不再默认堆叠入场、广告屏、技术状态和状态记录详情。
- `设备概览` 当前展示 `设备编号 / 点位 / 设备类别`，不再展示 `部署类型`。
- `设备状态` 展示设备运行、停卖、心跳、入场状态、当前异常摘要和历史异常记录数。
- `运营摘要` 汇总当前点位、支付方式、节能模式、网络信号、最近点位变更和记录摘要。
- 右侧分为 `设备操作` 和 `详细信息` 两组：设备操作包含 `远程操作 / 温度报警设置 / 状态记录 / 编辑状态 / 物料页面`；详细信息包含 `入场详情 / 广告屏信息 / 技术状态 / 记录摘要`。
- 点击右侧详细信息入口会打开二级浮层，二级浮层中的详情默认展开；从 `记录摘要` 继续点击 `查看状态记录` 时会先关闭摘要浮层，避免浮层层级错乱。

![F-005 设备详情摘要](../screenshots/device-prd/uf004-detail-summary-latest.png)
...
**当前页面表现：**
- 不再占用设备详情左侧首屏，用户从右侧 `详细信息 > 技术状态` 打开。
- 技术状态在二级浮层中以独立详情区展示，并默认展开。
...
**当前页面表现：**
- 不再默认占用详情左侧首屏，用户从右侧 `详细信息 > 入场详情` 打开。
- 信息在二级浮层中以单一详情区块展示，并默认展开。
- 支持查看点位图片并点击预览。
...
**当前页面表现：**
- 不再默认占用详情左侧首屏，用户从右侧 `详细信息 > 广告屏信息` 打开。
- 左侧支持图片和 H.264 视频。
- 右侧仅支持图片。
- 详情中按左右拼接方式预览。
...
**当前页面表现：**
- 可从右侧 `设备操作 > 状态记录` 直接打开完整状态记录弹层。
- 也可从右侧 `详细信息 > 记录摘要` 查看异常 / 运维条数，再点击 `查看状态记录` 进入完整弹层。
- 从记录摘要进入完整弹层时，系统会关闭摘要浮层，保证完整状态记录位于最上层。
- 弹层固定为 `异常记录` 与 `运维记录` 双标签。
...
**功能说明：** 用户可以从右侧操作区直接修改设备状态，快速把系统状态同步到线下真实状态。
...
- [ ] 页面右上角提供 `+ 设备录入` 按钮，桌面端与移动端悬浮按钮都跳转到设备录入页面。
...
1. 用户在设备管理页点击 `+ 设备录入`。
2. 系统跳转到 `设备入场` 页面。
3. 系统按当前登录商户范围加载可入场设备、可选点位和可选员工。
4. 用户选择设备、点位和运维人员；如无合适点位，可先 `+ 新增点位` 并自动选中。
5. 用户补充 GPS、经纬度、点位地址、节能模式、设备日期、网络信号、维护时段和备注。
6. 用户选择至少一种支付方式，并按需上传广告屏素材和点位照片。
7. 用户点击 `提交入场` 完成录入并返回设备管理。

**验收标准：**
- [ ] 设备录入入口从设备管理页顶部按钮和移动端悬浮按钮都可以进入。
- [ ] 录入页按当前登录商户范围过滤 `设备`、`点位` 和 `员工`，不再单独提供“所属客户”选择框。
...
![UF-002 添加设备并提交入场信息（最新分支结构预览）](../screenshots/device-prd/uf011-device-entry-latest.svg)
...
2. 系统打开设备详情弹层。
3. 用户先浏览设备概览、设备状态和运营摘要。
4. 用户根据右侧入口查看入场详情、广告屏信息、技术状态或记录摘要。
5. 用户在右侧设备操作区发起后续动作。

**验收标准：**
- [ ] 详情使用独立弹层，桌面端为超宽布局，移动端回退为单列卡片布局。
- [ ] 详情整体为 `左侧主内容 + 右侧设备操作区` 结构。
- [ ] `设备概览` 至少展示 `设备编号`、`点位`、`设备类别`；当前不再展示 `部署类型`。
- [ ] `设备状态` 至少展示 `设备状态`、`停卖状态`、`最近心跳`、`入场状态`、`当前异常摘要`。
- [ ] `运营摘要` 至少展示 `当前点位`、`支付方式`、`节能模式`、`网络信号`、`最近变更`、`记录摘要`。
- [ ] 右侧 `设备操作` 区固定包含 `远程操作`、`温度报警设置`、`状态记录`、`编辑状态`、`物料页面`。
- [ ] 右侧 `详细信息` 区固定包含 `入场详情`、`广告屏信息`、`技术状态`、`记录摘要`。

**参考截图：**

![UF-005 设备详情摘要](../screenshots/device-prd/uf004-detail-summary-latest.png)
...
**主流程：**
1. 用户打开某台设备的详情弹层。
2. 用户在右侧 `详细信息` 区点击 `技术状态`。
3. 系统打开技术状态二级浮层，并默认展开详情内容。
4. 系统展示当前设备的温度、湿度、软件版本和机构状态标签。
5. 用户根据读到的技术信息，决定继续查看记录、修改状态或发起远程操作。

**验收标准：**
- [ ] `技术状态` 必须从右侧 `详细信息` 入口打开，不再默认占用左侧首屏。
...
**主流程：**
1. 用户在右侧 `详细信息` 区点击 `入场详情`。
2. 系统打开入场详情二级浮层，展示基础信息、补充信息、点位图片和点位变更记录。
3. 用户点击详情底部 `编辑入场信息`。
4. 用户在编辑弹层中修改字段、上传图片并保存。

**验收标准：**
- [ ] 入场信息以单一折叠区块展示，不再拆成“核心信息 / 全部信息”两个入口。
- [ ] 入场信息通过右侧 `详细信息 > 入场详情` 打开，不再默认占用左侧首屏。
...
**主流程：**
1. 用户在右侧 `详细信息` 区点击 `广告屏信息`。
2. 页面按左右固定区域展示广告屏预览。
3. 用户在入场信息编辑弹层中上传或替换广告屏素材。
4. 用户保存后返回详情查看左右屏最新状态。

**验收标准：**
- [ ] 广告屏信息通过右侧 `详细信息 > 广告屏信息` 打开，并以 `左侧菜单` 和 `右侧排队号背景` 两组固定区域展示。
...
**主流程：**
1. 用户可以直接点击右侧 `设备操作 > 状态记录`。
2. 用户也可以点击右侧 `详细信息 > 记录摘要`，先查看异常记录数和运维记录数。
3. 用户在记录摘要中点击 `查看状态记录`。
4. 系统关闭记录摘要浮层，打开完整状态记录弹层，默认停留在 `异常记录` 标签。
5. 用户切换到 `运维记录` 标签查看补料 / 清洁等记录。

**验收标准：**
- [ ] `记录摘要` 通过右侧 `详细信息` 入口打开，只展示异常数量、运维数量和查看入口。
- [ ] 从 `记录摘要` 点击 `查看状态记录` 时，旧摘要浮层必须关闭，完整状态记录弹层必须位于最上层。
- [ ] 状态记录弹层固定为 `异常记录` 与 `运维记录` 双标签结构。
...
- FR-2：设备管理页必须提供 `+ 设备录入` 入口，并统一跳转至设备录入页面。
...
- FR-11：设备详情必须采用 `左主内容 + 右侧操作区` 布局。
- FR-12：设备概览必须展示 `设备编号`、`点位`、`设备类别`，当前不再展示 `部署类型`。
- FR-13：设备状态卡必须展示 `设备状态`、`停卖状态`、`最近心跳`、`入场状态`、`当前异常摘要`。
- FR-14：设备详情左侧首屏必须只保留 `设备概览`、`设备状态`、`运营摘要` 三类信息。
- FR-15：右侧 `设备操作` 必须包含 `远程操作`、`温度报警设置`、`状态记录`、`编辑状态`、`物料页面`。
- FR-16：右侧 `详细信息` 必须包含 `入场详情`、`广告屏信息`、`技术状态`、`记录摘要`。
- FR-17：技术状态必须通过右侧 `详细信息 > 技术状态` 打开，并展示温度、湿度、版本和机构状态。
- FR-18：入场信息必须通过右侧 `详细信息 > 入场详情` 打开，并支持点位图片预览和点位变更记录查看。
...
- FR-26：从 `记录摘要` 点击 `查看状态记录` 时，必须先关闭摘要浮层，避免弹层顺序错乱。
...
| 详细信息入口 | 点击 `入场详情 / 广告屏信息 / 技术状态 / 记录摘要` | 打开二级浮层，并默认展开对应详情内容 |
| 点位变更记录 | 多条历史点位变更 | 默认显示折叠摘要卡片，点击单条后展示完整变更前 / 变更后 |
| 记录摘要跳转 | 在记录摘要中点击 `查看状态记录` | 先关闭记录摘要浮层，再打开完整状态记录弹层 |
...
- 设备详情左侧不再默认展示完整入场、广告屏、技术状态和状态记录内容；这些信息需要通过右侧 `详细信息` 入口打开。
