...
test('设备详情桌面端应采用左主内容与右侧设备操作布局', () => {
  ...
  assert.ok(/openDetailQuickRestart\('\$\{escapeHtml\(summary\.deviceId \|\| ''\)\}', '重启系统'\)/.test(devicesHtml));
  assert.ok(/openDetailRestartOptions\('\$\{escapeHtml\(summary\.deviceId \|\| ''\)\}'\)/.test(devicesHtml));
...
test('设备状态卡应只保留运营判断字段，技术信息下沉到技术状态', () => {
  ...
  assert.ok(!/历史异常记录/.test(statusCardFn));
...
test('设备详情只在状态记录模块统计异常和运维条数，避免摘要区重复', () => {
  const statusMatch = devicesHtml.match(/function\s+renderDeviceStatusCard\s*\([\s\S]*?\n\s*}\n\n\s*function\s+renderDeviceOperationSummaryCard/);
  const operationMatch = devicesHtml.match(/function\s+renderDeviceOperationSummaryCard\s*\([\s\S]*?\n\s*}\n\n\s*function\s+renderTechnicalStatusCard/);
  const recordsMatch = devicesHtml.match(/function\s+renderStatusRecordsCard\s*\([\s\S]*?\n\s*}\n\n\s*function\s+getDetailInfoPanelContent/);
  assert.ok(statusMatch);
  assert.ok(operationMatch);
  assert.ok(recordsMatch);

  assert.ok(!/历史异常记录/.test(statusMatch[0]));
  assert.ok(!/记录摘要/.test(operationMatch[0]));
  assert.ok(!/运维\s*\$\{operationCount\}/.test(operationMatch[0]));
  assert.ok(/异常记录/.test(recordsMatch[0]));
  assert.ok(/运维记录/.test(recordsMatch[0]));
  assert.ok(/查看状态记录/.test(recordsMatch[0]));
  assert.ok(!/展开状态记录/.test(recordsMatch[0]));
  assert.ok(!/<details\s+class="detail-disclosure">/.test(recordsMatch[0]));
});
