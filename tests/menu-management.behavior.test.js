...
test('语言管理：切换到新设备时应自动初始化默认语言配置', () => {
  ...
  assert.strictEqual(ctx.deviceConfig.RCK385.langNames.zh, '中文');
  assert.strictEqual(ctx.deviceConfig.RCK385.langNames.en, 'English');
...
test('复制商品第一步进入第二步时，应带入多选目标分类', () => {
  ...
  const selected = ctx.getSelectedProductModalCategories().sort();
  assert.strictEqual(selected.join(','), '新品,经典');
});
...
test('复制商品：应支持一次选择多个目标设备并生成批量复制结果', () => {
  const ctx = loadMenuContext();
  ...
  ctx.setProductModalSelectedCategories(['新品']);
  nameInput.value = '批量复制商品';
  ...
  const queueMap = ctx.getCopyProductQueueMap();
  assert.strictEqual(Array.isArray(queueMap.RCK112), true);
  assert.strictEqual(Array.isArray(queueMap.RCK113), true);
  assert.strictEqual(queueMap.RCK112.length, 1);
  assert.strictEqual(queueMap.RCK113.length, 1);
  assert.strictEqual(ctx.document.getElementById('copyProductBatchResultModal').classList.contains('active'), true);
});
