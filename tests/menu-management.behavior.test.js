...
function loadMenuContext(options = {}) {
  ...
  let script = match[1]
    ...
    .replace(/let currentMenuInnerTab = '(settings|manage|latteArt|batch)';/, (_, tab) => `globalThis.currentMenuInnerTab = '${tab}';`)
...
test('商品管理页应接入共享员工权限脚本和商品设备范围', () => {
  ...
  assert.ok(/getModuleVisibleDeviceIds\([^)]*'products'/.test(html));
});
...
test('设备搜索：员工商品设备范围应限制可切换设备', () => {
    ...
    staffAccessHelper: {
      resolveCurrentStaffAccess() {
        return {
          isScoped: true,
          currentStaff: {
            permissions: ['ops.products'],
            devices: ['RCK386', 'RCK385'],
            moduleDeviceScopes: {
              products: { mode: 'custom', deviceIds: ['RCK385'] }
            }
          }
        };
      },
  ...
  assert.deepStrictEqual(Array.from(options), ['RCK385']);
});

test('设备搜索：本地 devicesData 只有两台时仍应补齐共享设备基线', () => {
  const ctx = loadMenuContext();
  const sharedDeviceIds = getSharedRuntimeDeviceIds(ctx);
  ctx.localStorage.setItem('devicesData', JSON.stringify([
    { id: 'RCK386', merchant: 'mer001', location: 'k8298', status: 'operational', sales: 'enabled', heartbeat: '自定义心跳', customFlag: 'keep-me' },
    { id: 'RCK385', merchant: 'mer001', location: 'k8298', status: 'faulted', sales: 'enabled', heartbeat: '自定义心跳-2' }
  ]));

  const runtimeDevices = ctx.loadRuntimeDeviceRecords();
  const runtimeIds = runtimeDevices.map(device => device.id);

  assert.deepStrictEqual(runtimeIds, sharedDeviceIds);
  assert.strictEqual(runtimeDevices[0].customFlag, 'keep-me');
  assert.strictEqual(runtimeDevices[0].heartbeat, '自定义心跳');
});
...
test('菜单管理应支持从商品详情 URL 自动打开点单屏预览并接收草稿消息', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'menu-management.html'), 'utf8');
  assert.ok(/params\.get\('openOrderPreview'\)\s*===\s*'1'/.test(html));
  assert.ok(/params\.get\('embedOrderPreview'\)\s*===\s*'1'/.test(html));
  assert.ok(/function\s+bindOrderPreviewDraftListener\s*\(/.test(html));
  assert.ok(/window\.addEventListener\('message'/.test(html));
  assert.ok(/message\.type\s*!==\s*'orderPreviewDraftPayload'/.test(html));
  assert.ok(/function\s+applyOrderPreviewDraftPayload\s*\(/.test(html));
});
...
test('菜单管理主页面应按菜单管理、印花图片设置、基本设置、批量改价顺序展示四个tab', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'menu-management.html'), 'utf8');
  const latteArtIndex = html.indexOf('id="menuInnerTabLatteArtBtn"');
...
test('菜单管理内层tab顺序应为菜单管理、印花图片设置、基本设置、批量改价', () => {
  ...
  assert.ok(latteArtIndex >= 0, '缺少印花图片设置 tab');
  assert.ok(settingsIndex >= 0, '缺少基本设置 tab');
  assert.ok(batchIndex >= 0, '缺少批量改价 tab');
  assert.ok(manageIndex < latteArtIndex, '菜单管理应排在印花图片设置前');
  assert.ok(latteArtIndex < settingsIndex, '印花图片设置应排在基本设置前');
  assert.ok(settingsIndex < batchIndex, '基本设置应排在批量改价前');
});

test('印花图片设置tab应提供当前设备作用域、总计统计、上传入口和复制弹层', () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'menu-management.html'), 'utf8');
  assert.ok(html.includes('印花图片设置'));
  assert.ok(html.includes('id="menuLatteArtPanel"'));
  assert.ok(html.includes('id="menuLatteArtScopeSummary"'));
  assert.ok(html.includes('总计印花数'));
  assert.ok(html.includes('id="menuLatteArtSearchInput"'));
  assert.ok(html.includes('上传印花图片'));
  assert.ok(!html.includes('更换图片'));
  assert.ok(!html.includes('改名'));
  assert.ok(!html.includes('renameMenuLatteArtItem'));
  assert.ok(html.includes('id="menuLatteArtUploadModal"'));
  assert.ok(html.includes('id="menuLatteArtCopyModal"'));
});
...
test('菜单管理页初始化时应支持通过 innerTab 参数选中印花图片设置', () => {
  const ctx = loadMenuContext();
  ctx.window.location.search = '?tab=menu&innerTab=latteArt';
  ctx.init();

  assert.strictEqual(ctx.currentMenuInnerTab, 'latteArt');
  assert.strictEqual(ctx.document.getElementById('menuInnerTabLatteArtBtn').classList.contains('active'), true);
  assert.strictEqual(ctx.document.getElementById('menuInnerTabManageBtn').classList.contains('active'), false);
  assert.strictEqual(ctx.document.getElementById('menuInnerTabSettingsBtn').classList.contains('active'), false);
});
...
test('设备搜索：应支持按点位名称匹配设备并展示“点位名称 · 设备编号”', () => {
  ...
  ctx.localStorage.setItem('devicesData', JSON.stringify([
    ...
    { id: 'RCK112', location: 'k1002', entered: true, entryInfo: { locationName: '北京望京测试店' } }
  ]));
  ctx.localStorage.setItem('locationsData', JSON.stringify([
    { code: 'k1001', name: '上海国家会展中心A馆' },
    { code: 'k1002', name: '北京望京测试店' }
  ]));
  ctx.currentDevice = 'RCK111';
  ...
  assert.strictEqual(ctx.matchDeviceByInput('北京望京'), 'RCK112');
  ...
  ctx.document.getElementById('deviceSearchInput').value = '北京望京测试店';
  ctx.changeDeviceByInput();

  assert.strictEqual(ctx.currentDevice, 'RCK112');
  assert.strictEqual(ctx.document.getElementById('deviceSearchInput').value, '北京望京测试店 · RCK112');
});
...
test('印花图片设置：切换设备后应刷新为新设备的作用域和素材', () => {
  const ctx = loadMenuContext();
  ctx.showToast = () => {};
  ctx.localStorage.setItem('devicesData', JSON.stringify([
    { id: 'RCK111', location: 'k1001' },
  ...
  ctx.localStorage.setItem('deviceLatteArtLibrary_RCK111', JSON.stringify({
    version: 1,
    deviceId: 'RCK111',
    items: [{ id: 'art_1', name: '天鹅', nameKey: '天鹅', image: 'img-1', sourceDeviceId: 'RCK111' }]
  }));
  ctx.localStorage.setItem('deviceLatteArtLibrary_RCK112', JSON.stringify({
    version: 1,
    deviceId: 'RCK112',
    items: [{ id: 'art_2', name: '玫瑰', nameKey: '玫瑰', image: 'img-2', sourceDeviceId: 'RCK112' }]
  }));
  ctx.currentDevice = 'RCK111';

  ctx.initDeviceSearch();
  ctx.switchMenuInnerTab('latteArt');
  ctx.document.getElementById('deviceSearchInput').value = '北京望京测试店';

  ctx.changeDeviceByInput();

  const panelHtml = ctx.document.getElementById('menuLatteArtPanel').innerHTML;
  assert.strictEqual(ctx.currentDevice, 'RCK112');
  assert.ok(panelHtml.includes('当前设备生效 · RCK112'));
  assert.ok(panelHtml.includes('玫瑰'));
  assert.ok(!panelHtml.includes('天鹅'));
});
...
test('印花图片设置：损坏的本地存储应安全回退为空库', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${ctx.currentDevice}`, '{bad json');

  const library = ctx.readStoredMenuLatteArtLibrary(ctx.currentDevice);

  assert.deepStrictEqual(JSON.parse(JSON.stringify(library.items)), []);
});

test('印花图片设置：当前设备上传同名图片时应阻止并提示先删除再上传', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  const toastMessages = [];
  ctx.showToast = (message) => {
  ...
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${ctx.currentDevice}`, JSON.stringify({
    version: 1,
    deviceId: ctx.currentDevice,
    items: [{ id: 'art_1', name: '天鹅', nameKey: '天鹅', image: 'old', sourceDeviceId: ctx.currentDevice }]
  }));

  const result = ctx.saveMenuLatteArtUpload({
    deviceId: ctx.currentDevice,
    name: ' 天鹅 ',
    image: 'new'
  });

  const library = JSON.parse(ctx.localStorage.getItem(`deviceLatteArtLibrary_${ctx.currentDevice}`) || '{}');
  assert.strictEqual(result, false);
  assert.strictEqual(library.items[0].image, 'old');
  assert.ok(toastMessages.some(message => message.includes('请先删除后再上传')));
});

test('印花图片设置tab：切换后应渲染当前设备作用域和总计印花数', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  ctx.productsData = {
    拉花系列: {
  ...
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${ctx.currentDevice}`, JSON.stringify({
    ...
    deviceId: ctx.currentDevice,
    items: [
      { id: 'art_1', name: '天鹅', nameKey: '天鹅', image: 'img', sourceDeviceId: ctx.currentDevice },
      { id: 'art_2', name: '随机图', nameKey: '随机图', image: 'img2', sourceDeviceId: ctx.currentDevice }
    ]
  }));
  ...
  const panelHtml = ctx.document.getElementById('menuLatteArtPanel').innerHTML;
  assert.ok(panelHtml.includes('当前设备生效'));
  assert.ok(panelHtml.includes('总计印花数'));
  assert.ok(panelHtml.includes('<strong>2</strong>'));
  assert.ok(panelHtml.includes('缺失素材'));
  assert.ok(!panelHtml.includes('更换图片'));
  assert.ok(!panelHtml.includes('改名'));
  assert.ok(!panelHtml.includes('名称键：'));
  assert.ok(!panelHtml.includes('当前没有商品引用这个名称'));
  assert.ok(!panelHtml.includes('已联动'));
  assert.ok(!panelHtml.includes('未引用'));
  assert.ok(!panelHtml.includes('印花名称用于和商品配置联动'));
});

test('印花图片设置：保存成功后应提示是否复制到其他设备', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  ...
  const saved = ctx.saveMenuLatteArtUpload({
    deviceId: ctx.currentDevice,
    name: '天鹅',
    image: 'img'
  });
  ...
  assert.ok(confirmMessages.some(message => message.includes('是否复制到其他设备')));
});

test('印花图片设置：只有当前一台设备时保存后不应提示复制', () => {
  const ctx = loadMenuContext();
  ctx.window.COFE_SHARED_MOCK_DATA.helpers.resolveDevices = (storedDevices) => JSON.parse(JSON.stringify(storedDevices || []));
  ctx.localStorage.setItem('devicesData', JSON.stringify([
    { id: 'RCK111', location: 'k1001' }
  ]));
  ctx.localStorage.setItem('locationsData', JSON.stringify([
  ...
  assert.strictEqual(ctx.document.getElementById('menuLatteArtCopyModal').classList.contains('active'), false);
});

test('印花图片设置：只有当前一台设备时不应显示复制入口', () => {
  const ctx = loadMenuContext();
  ctx.window.COFE_SHARED_MOCK_DATA.helpers.resolveDevices = (storedDevices) => JSON.parse(JSON.stringify(storedDevices || []));
  ctx.localStorage.setItem('devicesData', JSON.stringify([
    { id: 'RCK111', location: 'k1001' }
  ...
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${ctx.currentDevice}`, JSON.stringify({
    version: 1,
    deviceId: ctx.currentDevice,
    items: [{ id: 'art_1', name: '天鹅', nameKey: '天鹅', image: 'img', sourceDeviceId: ctx.currentDevice }]
  }));

  ctx.switchMenuInnerTab('latteArt');

  const panelHtml = ctx.document.getElementById('menuLatteArtPanel').innerHTML;
  assert.ok(!panelHtml.includes('复制到其他设备'));
});

test('印花图片设置：复制预检查应区分覆盖和新增', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  const deviceIds = getSharedRuntimeDeviceIds(ctx);
  const sourceDeviceId = deviceIds[0];
  const overwriteDeviceId = deviceIds[1];
  const addDeviceId = deviceIds[2];
  ctx.currentDevice = sourceDeviceId;

  ctx.localStorage.setItem(`deviceLatteArtLibrary_${sourceDeviceId}`, JSON.stringify({
    version: 1,
    deviceId: sourceDeviceId,
    items: [{ id: 'art_saved', name: '天鹅', nameKey: '天鹅', image: 'fresh', sourceDeviceId }]
  }));
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${overwriteDeviceId}`, JSON.stringify({
    version: 1,
    deviceId: overwriteDeviceId,
    items: [{ id: 'art_old', name: '天鹅', nameKey: '天鹅', image: 'old', sourceDeviceId: overwriteDeviceId }]
  }));
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${addDeviceId}`, JSON.stringify({
    version: 1,
    deviceId: addDeviceId,
    items: []
  }));

  const preview = ctx.previewMenuLatteArtCopyImpact('art_saved', [overwriteDeviceId, addDeviceId]);

  assert.strictEqual(preview.overwriteCount, 1);
  assert.strictEqual(preview.addCount, 1);
});

test('印花图片设置：复制执行后应覆盖同名素材并新增缺失素材', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  const deviceIds = getSharedRuntimeDeviceIds(ctx);
  const sourceDeviceId = deviceIds[0];
  ...
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${overwriteDeviceId}`, JSON.stringify({
    ...
    deviceId: overwriteDeviceId,
    items: [
      { id: 'art_old', name: '天鹅', nameKey: '天鹅', image: 'old', sourceDeviceId: overwriteDeviceId },
      { id: 'art_keep', name: '爱心', nameKey: '爱心', image: 'keep', sourceDeviceId: overwriteDeviceId }
    ]
  }));
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${addDeviceId}`, JSON.stringify({
    version: 1,
  ...
  const results = ctx.copyMenuLatteArtItemToDevices('art_saved', [overwriteDeviceId, addDeviceId]);
  const overwrittenLibrary = JSON.parse(ctx.localStorage.getItem(`deviceLatteArtLibrary_${overwriteDeviceId}`) || '{}');
  const addedLibrary = JSON.parse(ctx.localStorage.getItem(`deviceLatteArtLibrary_${addDeviceId}`) || '{}');

  assert.ok(results.some(item => item.deviceId === overwriteDeviceId && item.action === 'overwritten'));
  assert.ok(results.some(item => item.deviceId === addDeviceId && item.action === 'added'));
  assert.strictEqual(overwrittenLibrary.items.find(item => item.nameKey === '天鹅').image, 'fresh');
  assert.strictEqual(overwrittenLibrary.items.find(item => item.nameKey === '爱心').image, 'keep');
  assert.strictEqual(addedLibrary.items.length, 1);
});

test('印花图片设置：开始复制成功后应自动关闭复制弹窗', () => {
  const ctx = loadMenuContext();
  ctx.initDeviceSearch();
  const deviceIds = getSharedRuntimeDeviceIds(ctx);
  const sourceDeviceId = deviceIds[0];
  const targetDeviceId = deviceIds[1];
  ctx.currentDevice = sourceDeviceId;
  ctx.showToast = () => {};
  ...
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${sourceDeviceId}`, JSON.stringify({
    version: 1,
    deviceId: sourceDeviceId,
    items: [{ id: 'art_saved', name: '测试', nameKey: '测试', image: 'fresh', sourceDeviceId }]
  }));
  ctx.localStorage.setItem(`deviceLatteArtLibrary_${targetDeviceId}`, JSON.stringify({
    version: 1,
    deviceId: targetDeviceId,
    items: []
  }));

  ctx.openMenuLatteArtCopyModal('art_saved');
  ctx.toggleMenuLatteArtCopyTargetSelection(targetDeviceId, true);
  ctx.submitMenuLatteArtCopy();
