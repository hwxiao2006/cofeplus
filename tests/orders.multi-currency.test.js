...
test('订单金额展示应按订单币种渲染，统计区支持按币种汇总', () => {
  ...
  assert.ok(/id="filteredTotalAmount"/.test(html), 'toolbar should include filtered total amount node');
  assert.ok(/id="successfulTotalAmount"/.test(html), 'toolbar should include successful amount node');
  assert.ok(/function\s+formatOrderTotalsByCurrency\s*\(orderList,\s*successfulOnly = false\)/.test(html), 'multi-currency formatter should support all or successful-only totals');
  assert.ok(/if\s*\(successfulOnly\s*&&\s*order\?\.paymentStatus\s*!==\s*'succeed'\)\s*return;/.test(html), 'amount formatter should optionally filter to successful payments');
  assert.ok(/document\.getElementById\('filteredTotalAmount'\)\.textContent\s*=\s*formatOrderTotalsByCurrency\(filteredData\)/.test(html));
  assert.ok(/document\.getElementById\('successfulTotalAmount'\)\.textContent\s*=\s*formatOrderTotalsByCurrency\(filteredData,\s*true\)/.test(html));
