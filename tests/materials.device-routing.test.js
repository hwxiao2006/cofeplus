...
test('物料页：页头副标题应显示当前设备名称并移除语言文案', () => {
  assert.ok(/id="pageSubtitle"/.test(html));
  assert.ok(!/语言=中文/.test(html));
  assert.ok(/function\s+resolveDeviceTitleName\s*\(deviceId\)\s*\{/.test(html));
  assert.ok(/function\s+syncDeviceContext\s*\(deviceId\)\s*\{[\s\S]*pageSubtitle/.test(html));
});
...
test('物料页：确认补充后应写入设备运维记录', () => {
  ...
  assert.ok(/operatorName,\s*[\s\S]*operatorPhone,/.test(html));
  assert.ok(/maintainerName:\s*operatorName/.test(html));
  assert.ok(/maintainerPhone:\s*operatorPhone/.test(html));
  assert.ok(/createRefillMaintenanceRecord\(material,\s*oldRemaining,\s*nextCurrent\)/.test(html));
});

test('物料页：写入运维记录前应优先使用当前登录操作人员和电话', () => {
  assert.ok(/function\s+pickFirstValidValue\s*\(/.test(html));
  assert.ok(/function\s+resolveCurrentOperationUser\s*\(\)/.test(html));
  assert.ok(/JSON\.parse\(localStorage\.getItem\(LOGIN_SESSION_KEY\)/.test(html));
  assert.ok(/JSON\.parse\(localStorage\.getItem\(SIDEBAR_LOGIN_PROFILE_KEY\)/.test(html));
  assert.ok(/function\s+resolveOperatorByDevice\s*\(deviceId\)\s*\{[\s\S]*entryInfo\.operatorPhone[\s\S]*entryInfo\.mobile/.test(html));
  assert.ok(/const\s+operator\s*=\s*resolveCurrentOperationUser\(\);/.test(html));
  assert.ok(/const\s+fallbackOperator\s*=\s*resolveOperatorByDevice\(currentDevice\);/.test(html));
...
test('物料页：补充弹窗中当前量仅允许整数', () => {
  ...
  assert.ok(/if\s*\(!Number\.isInteger\(refillDelta\)\)/.test(html));
});

test('物料页：补充弹窗应按原流程在现有值上增加输入值，并提供重置为 0', () => {
  assert.ok(/<label class="value-edit-label" for="adjustCurrent">补充数量<\/label>/.test(html));
  assert.ok(/id="adjustCurrent"[^>]*onfocus="selectNumericInputValue\(this\)"/.test(html));
  assert.ok(/function\s+selectNumericInputValue\s*\(input\)/.test(html));
  assert.ok(/document\.getElementById\('adjustCurrent'\)\.value = 0;/.test(html));
  assert.ok(/function\s+resetRefillCurrentToZero\s*\(/.test(html));
  assert.ok(/currentRefillResetMode\s*=\s*true;/.test(html));
  assert.ok(/const\s+refillDelta\s*=\s*Number\(document\.getElementById\('adjustCurrent'\)\.value\);/.test(html));
  assert.ok(/const\s+nextCurrent\s*=\s*currentRefillResetMode\s*\?\s*0\s*:\s*oldRemaining\s*\+\s*refillDelta;/.test(html));
  assert.ok(/onclick="closeRefillModal\(\)">取消<\/button>\s*<button class="btn btn-default" onclick="resetRefillCurrentToZero\(\)">重置为 0<\/button>\s*<button class="btn btn-confirm" onclick="confirmRefill\(\)">确认保存<\/button>/.test(html));
  assert.ok(/重置为 0/.test(html));
});
...
test('物料页：货道名称不应被状态和编辑按钮挤压隐藏', () => {
  assert.ok(/\.material-card-title-row\s*\{[\s\S]*flex-wrap:\s*wrap;/.test(html));
  assert.ok(/\.material-name\s*\{[\s\S]*flex:\s*1 1 96px;/.test(html));
  assert.ok(/\.material-card-tools\s*\{[\s\S]*flex-wrap:\s*wrap;/.test(html));
});
...
test('物料页：发货跳转前应写入来源感知返回上下文', () => {
  ...
  assert.ok(/const\s+associatedMaterialCode\s*=\s*matchedLane\?\.materialCode\s*\|\|\s*matchedLane\?\.materialNumber\s*\|\|\s*materialCode\s*\|\|\s*null/.test(html));
  assert.ok(/materialCode:\s*associatedMaterialCode/.test(html));
...
test('物料页：编辑货道关联物料应按商户物料表和独立权限控制', () => {
  assert.ok(/const\s+LANE_MATERIAL_OVERRIDE_STORAGE_KEY\s*=\s*'materialsLaneMaterialOverrides';/.test(html));
  assert.ok(/const\s+MERCHANT_MATERIAL_MASTER_DATA\s*=/.test(html));
  assert.ok(/function\s+getCurrentMerchantContext\s*\(/.test(html));
  assert.ok(/function\s+getCurrentMerchantMaterialOptions\s*\(/.test(html));
  assert.ok(/function\s+canEditLaneMaterial\s*\(/.test(html));
  assert.ok(/ops\.materials\.laneMaterialEdit/.test(html));
  assert.ok(/id="laneMaterialModal"/.test(html));
  assert.ok(/编辑关联物料/.test(html));
  assert.ok(/materialsLaneMaterialOverrides/.test(html));
  assert.ok(/merchantId:\s*getCurrentMerchantContext\(\)\.merchantId/.test(html));
  assert.ok(/currentDevice/.test(html));
...
test('物料页：编辑关联物料应使用补充物料页同类的分类搜索卡片选择器', () => {
  ...
  assert.ok(/id="laneMaterialSearch"/.test(html));
  assert.ok(/id="laneMaterialList"/.test(html));
  assert.ok(/const\s+LANE_MATERIAL_CATEGORY_ORDER\s*=/.test(html));
  assert.ok(/function\s+switchLaneMaterialCategory\s*\(/.test(html));
  assert.ok(/function\s+selectLaneMaterial\s*\(/.test(html));
  assert.ok(/lane-material-option-card/.test(html));
  assert.ok(/lane-material-option-card selected/.test(html));
  assert.ok(!/id="laneMaterialSelect"/.test(html));
  assert.ok(!/REFILL_CATEGORY_ORDER/.test(html));
});

test('物料页：编辑关联物料应限制为当前货道大类匹配的物料分类', () => {
  assert.ok(/const\s+LANE_GROUP_TO_MATERIAL_CATEGORIES\s*=/.test(html));
  assert.ok(/咖啡豆仓:\s*\['奶&咖啡&水'\]/.test(html));
  assert.ok(/'牛奶&水':\s*\['奶&咖啡&水'\]/.test(html));
  assert.ok(/糖浆:\s*\['糖浆'\]/.test(html));
  assert.ok(/前后道粉:\s*\['前\/后道粉'\]/.test(html));
  assert.ok(/包材:\s*\['包材'\]/.test(html));
  assert.ok(/制冰机:\s*\['食材'\]/.test(html));
  assert.ok(/function\s+getAllowedMaterialCategoriesForLane\s*\(/.test(html));
  assert.ok(/getAllowedMaterialCategoriesForLane\(laneItem\)/.test(html));
  assert.ok(/allowedCategories\.includes\(material\.category\)/.test(html));
});
