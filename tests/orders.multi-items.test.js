...
test('订单页应通过 fallback 订单生成器保留至少 20 笔订单和多商品场景', () => {
  assert.ok(/const\s+MIN_ORDER_RECORDS\s*=\s*20;/.test(html));
  assert.ok(/const\s+FALLBACK_MULTI_ITEM_ORDER_COUNT\s*=\s*\d+;/.test(html));
