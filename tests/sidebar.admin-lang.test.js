...
test('所有侧栏页面都应提供独立的后台菜单语言切换', () => {
  sidebarPages.forEach(file => {
    ...
    assert.ok(/\.sidebar-meta-row\s*\{[\s\S]*padding-left:\s*calc\(var\(--sidebar-nav-padding-x\)\s*\+\s*var\(--sidebar-section-title-padding-x\)\s*-\s*var\(--sidebar-header-padding-x\)\);/.test(css), `${file} 元信息行未与菜单文字对齐`);
  });
});
