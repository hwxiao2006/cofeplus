...
test('人员管理页：添加人员表单应移除授权与 openId 输入，并暂时隐藏公众号推送', () => {
  ...
  assert.ok(/先选择该人员可管理的设备，后续页面可见设备都从这里选择/.test(staffHtml));
  assert.ok(/默认继承全部可管理设备；只有切到“指定设备”时，才需要进一步选择/.test(staffHtml));
  ...
  assert.ok(staffHtml.includes('按模块快速勾选，一屏完成。'));
...
test('人员管理页：应支持按页面配置设备范围，并将范围限制在负责设备内', () => {
  ...
  assert.ok(/只显示已勾选且当前真正支持设备范围的页面；其他页面不在这里出现/.test(staffHtml));
  assert.ok(/默认继承全部可管理设备；只有切到“指定设备”时，才需要进一步选择/.test(staffHtml));
  assert.ok(/class="staff-modal-step-summary-list"/.test(staffHtml));
  assert.ok(/修改/.test(staffHtml));
  ...
  assert.ok(/function\s+resolveStaffModalValidationResult\s*\(/.test(staffHtml));
  assert.ok(/全部可管理设备/.test(staffHtml));
  assert.ok(/指定设备/.test(staffHtml));
  assert.ok(/重新选择/.test(staffHtml));
  assert.ok(/当前可查看设备：/.test(staffHtml));
  ...
  assert.ok(/页面已无可见设备，请重新选择范围或改回“全部可管理设备”/.test(staffHtml));
