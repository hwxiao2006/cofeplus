...
function buildSandbox() {
  ...
  [
    ...
    'getFaultRecordList',
    'buildMockFaultAbnormalRecords',
    'getFaultAbnormalRecords',
    ...
    'buildMaintenanceRecordContactFallback',
...
test('运行时：异常记录为空时应返回多条 mock 异常记录', () => {
  const sandbox = buildSandbox();
  const records = sandbox.getFaultAbnormalRecords('RCK386');
  assert.ok(records.length >= 3, '空记录时应至少返回 3 条 mock 异常记录');
  records.forEach((item, index) => {
    assert.strictEqual(item.deviceId, 'RCK386', `第 ${index + 1} 条设备编号应为 RCK386`);
    assert.ok(String(item.message || '').trim(), `第 ${index + 1} 条异常记录消息不能为空`);
  });
});

test('运行时：异常记录不足三条时应补齐 mock 且保留原记录', () => {
  const sandbox = buildSandbox();
  sandbox.localStorage.setItem('deviceFaultAbnormalRecords', JSON.stringify([
    {
      deviceId: 'RCK386',
    ...
    {
      deviceId: 'RCK999',
      time: '2026-03-05 11:10:00',
      code: 'OTHER-001',
      message: '其他设备异常'
    }
  ]));
  const records = sandbox.getFaultAbnormalRecords('RCK386');
  assert.ok(records.length >= 3, '单条真实异常时应补齐到多条');
  assert.ok(records.some(item => item.code === 'REAL-001' && item.message.includes('真实异常')), '应保留原始真实异常记录');
});

test('运行时：多条记录中部分缺失联系人字段时应自动回填', () => {
  const sandbox = buildSandbox();
  const mixedRecords = [
    {
      time: '2026-03-05 12:01:00',
  ...
  const records = sandbox.getMaintenanceRecordsByDevice('RCK386');
  assert.strictEqual(records.length, 2, '应命中 2 条 RCK386 运维记录');
  assert.strictEqual(records[0].operatorName, '李运维', '第一条记录应保留原始运维人员');
  assert.strictEqual(records[0].operatorPhone, '13912340000', '第一条记录应保留原始运维电话');
  assert.strictEqual(records[1].operatorName, '李运维', '第二条记录应回填运维人员');
  assert.strictEqual(records[1].operatorPhone, '13912340000', '第二条记录应回填运维电话');
});
