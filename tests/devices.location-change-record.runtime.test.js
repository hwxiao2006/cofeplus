...
test('运行时：renderLocationChangeRecords 应按时间倒序渲染点位变更前后值', () => {
  ...
  assert.ok(html.includes('<details class="detail-location-change-card">'), '记录应使用折叠卡片控制默认高度');
  assert.ok(html.includes('<summary class="detail-location-change-summary">'), '折叠卡片应提供摘要入口');
  assert.ok(html.includes('<span class="detail-location-change-count">3 项变更</span>'), '摘要应显示变更项数量');
  assert.ok(html.includes('点位名称、点位地址、经纬度'), '摘要应显示变更字段');
  assert.ok(html.includes('点位名称：B 点位 -> <strong>C 点位</strong>'), '摘要应突出主要变化');
