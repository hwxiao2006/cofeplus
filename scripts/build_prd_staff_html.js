#!/usr/bin/env node
/**
 * 从 tasks/prd-staff-management-user-flow.md 生成自包含 HTML 版本。
 *
 * - 截图内联为 base64 data URI，产物不依赖外部文件
 * - 目录只列顶级章节（## 级），符合 AGENTS.md 的紧凑目录要求
 * - 用法：node scripts/build_prd_staff_html.js
 */
const fs = require('fs');
const path = require('path');

const ROOT = path.join(__dirname, '..');
const MD_PATH = path.join(ROOT, 'tasks', 'prd-staff-management-user-flow.md');
const OUT_PATH = path.join(ROOT, 'tasks', 'prd-staff-management-user-flow.html');
...
<title>产品需求文档：人员管理（v2.3 分级授权与账号体系）</title>
<style>
...
console.log(`written ${OUT_PATH} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
