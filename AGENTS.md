...
- PRD custom domain `prd.cofeplus.dpdns.org` is a separate Cloudflare Pages direct-upload project. Updating files under `tasks/` and pushing `main` does not update that domain by itself. Deploys REPLACE the whole site — a bundle missing any PRD takes that PRD offline. Always rebuild the full bundle with `scripts/deploy_prd_pages.sh` (never hand-copy a subset into `prd-site/`), then deploy with `npx wrangler pages deploy prd-site --project-name cofeplus-prd --branch main --commit-dirty=true`, and verify the custom-domain URL. New PRDs also need a link added to `scripts/prd_site_index.html`. See `docs/prd-site-deployment.md` for the full walkthrough.
